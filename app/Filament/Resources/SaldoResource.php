            ...
            ->schema([
                Group::make()->schema([
                    Section::make('Transaction')->schema([
                        Select::make('kas_bank')
                            ...
                            ->options(function () {
                                return \App\Models\Transaction::where('status', 'pending')
                                    ->whereHas('penerima', function ($query) {
                                        // Filter berdasarkan 'bayar_dari' yang memiliki nilai '1-00001 - Kas'
                                        $query->where('bayar_dari', '1-00001 - Kas');
                                    })
                                    ->whereDoesntHave('saldo') // Filter transaksi yang belum digunakan
                                    ->pluck('penerima', 'id'); // Menampilkan 'penerima' sebagai label dan 'id' sebagai value
                            })
                            ->required()
                            ->reactive() // Menjadikan field reaktif
                            ->placeholder(function () {
                                // Logika untuk menentukan placeholder
                                $availableTransactions = \App\Models\Transaction::where('status', 'pending')
                                    ->whereHas('penerima', function ($query) {
                                        $query->where('bayar_dari', '1-00001 - Kas');
                                    })
                                    ->whereDoesntHave('saldo')
                                    ->exists(); // Mengecek apakah ada transaksi yang tersedia
                        
                                // Mengembalikan placeholder berdasarkan kondisi
                            ...
                            ->afterStateUpdated(function ($state, callable $set) {
                                if ($state) {
                                    ...
                                    $set('saldo_items', []);
                                    // Kosongkan penerima jika tidak ada transaksi yang dipilih
                                    $set('penerima', null);
                                }
                            })
                    ...
                    Select::make('kontak_id')
                        ...
                        ->disabled()
                        ->default(fn ($state, $record) => $record ? $record->nomor : app(\App\Models\Saldo::class)->generateKodeBank()),
                ...
                Group::make()->schema([
                    Section::make('Detail items')->schema([
                        Repeater::make('saldo_items')
                        ->label('Details Saldo')
                        ->relationship('saldo_items')
                        ->schema([
                            ...
                            ->afterStateUpdated(function ($state, callable $set, $get) {
                                ...
                                $subtotal = collect($get('saldo_items'))
                            ...
                            Section::make()->schema([
                                ...
                                ->content(function (Get $get, Set $set) {
                                    ...
                                    foreach ($repeaters as $key => $repeater) {
                                        $amount = (float) $get("saldo_items.{$key}.biaya");
                                        $total += $amount;
                                    }
                            
                                    // Set subtotal atau saldo_kas berdasarkan jenis transaksi
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                TextColumn::make('saldo_items')
                ->label('Keterangan')
                ->formatStateUsing(function ($record) {
                    // Mengambil data saldo_items dan hanya menampilkan nama kategori
                    $accounts = $record->saldo_items->map(function ($item) {
                ...
                ->tooltip(function ($record) {
                    ...
                    return $record->saldo_items->map(function ($item) {
                ...
                //     '<a href="' . url('storage/' . strtolower($state)) . '" download="' . strtolower($record->nama_member) . '.png">Download Gambar</a>')
                // ->html()                
                // ->toggleable(isToggledHiddenByDefault: true),
                ...
                Tables\Actions\ActionGroup::make([
                    ...
                    ->action(function (Saldo $record) {
                        ...
                        if ($record->kas_bank === 'Kirim Dana' && $transaction && $transaction->status === 'paid') {
                            return redirect()->route('saldo.voucher', $record->id);
                        }
                    ...
                    ->visible(fn (Saldo $record) => $record->kas_bank === 'Kirim Dana' && $record->transaction && $record->transaction->status === 'paid'),                
                    ...
                    Tables\Actions\Action::make('print')
                        ...
                        ->action(function (Saldo $record) {
                            ...
                            if ($record->kas_bank === 'Terima Dana') {
                                return redirect()->route('saldo.print', $record->id);
                            } 
                        ...
                        ->visible(fn (Saldo $record) => $record->kas_bank === 'Terima Dana'),
                        ...
                        ->action(function ($record) {
                            ...
                            // **1. Notifikasi Popup (Toast) untuk user**
                            Notification::make()
                                ->title('Status Lunas')
                            ...
                            //     Notification::make()
                            //         ->title('Status Lunas')
                            //         ->body("**{$penerima} - {$accountName}** Pembayaran transaksi Petty Cash berhasil diperbarui menjadi \"Sudah Dikirim\".")
                            //         ->success()
                            //         ->sendToDatabase($admin);
                            // }
                        })
                        ->requiresConfirmation()
                        ->visible(fn ($record) => optional($record->transaction)->status === 'pending'),                    
            ...
            ->bulkActions([
                Tables\Actions\BulkActionGroup::make([
                    ...
                    ->action(function (Collection $records) {
                        ...
                        return redirect()->route('laporan.kas', ['selected' => true]);
                    })
                    ->requiresConfirmation(),
